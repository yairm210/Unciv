...
internal object DesktopLauncher {
    ...
    private fun packImages() {
        ...
        // see ../Images/pack.json for the most important settings
        val settings = TexturePacker.Settings()

        val imageDir = "..${File.separator}Images"
        if (File(imageDir).exists())   // So we don't run this from within a fat JAR
            if (packImagesIfOutdated(settings, imageDir, ".", "game"))
                if (File("$imageDir${File.separator}H").exists()) {
                    val atlasFile = "game.atlas"
                    val atlasOut = mutableListOf<String>()
                    File(atlasFile).forEachLine {
                        atlasOut += if (it.length > 2 && it[1] in "/${File.separator}")
                            it.drop(2) else it
                    }
                    File(atlasFile).writeText(atlasOut.joinToString("\n"), Charset.forName ("UTF-8"))
                }
        ...
        if(modDirectory.exists()) {
            ...
            for (mod in modDirectory.listFiles()!!){
                val modImageDir = "${mod.path}${File.separator}Images"
                if (!mod.isHidden && File(modImageDir).exists())
                    packImagesIfOutdated(settings, modImageDir, mod.path, "game")
            }
        }
    ...
    private fun packImagesIfOutdated (settings: TexturePacker.Settings, input: String, output: String, packFileName: String): Boolean {
        ...
        if (atlasFile.exists() && File("$output${File.separator}$packFileName.png").exists()) {
            ...
            if (!File(input).listTree().any { it.extension in listOf("png","jpg","jpeg","json") && it.lastModified() > atlasModTime }) return false
        }
