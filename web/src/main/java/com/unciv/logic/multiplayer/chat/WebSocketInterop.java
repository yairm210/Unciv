...
final class WebSocketInterop {
    private WebSocketInterop() {
    }
    ...
    @JSBody(
            params = {"url", "protocols", "onOpen", "onMessage", "onError", "onClose"},
            script =
                    "try {\n"
                            + "  var ws = protocols && protocols.length ? new WebSocket(url, protocols) : new WebSocket(url);\n"
    ...
    @JSBody(params = {"runnable", "delayMs"}, script = "setTimeout(function(){ runnable.$run(); }, delayMs);")
