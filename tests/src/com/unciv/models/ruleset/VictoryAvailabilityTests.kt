...
class VictoryAvailabilityTests {
    ...
    private fun gameWithVictory(vararg uniques: String): TestGame {
        val testGame = TestGame()
        testGame.makeHexagonalMap(2)
        val uniquesJson = uniques.joinToString(", ") { "\"$it\"" }
        val victory = json().fromJson(Victory::class.java,
            """{ "name": "$victoryName", "uniques": [$uniquesJson], "milestones": ["Build [Monument]"] }""")
    ...
    fun `a victory unavailable to a civ is not presented to it as its own goal`() {
        ...
        Assert.assertTrue(victoryName in testGame.gameInfo.getEnabledVictories(human))
        Assert.assertFalse(victoryName in testGame.gameInfo.getEnabledVictories(ai))
        // Asked without a civilization, the enabled victories are the enabled victories
        Assert.assertTrue(victoryName in testGame.gameInfo.getEnabledVictories())
    }
