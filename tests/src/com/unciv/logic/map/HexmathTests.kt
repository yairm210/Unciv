...
class HexmathTests {
    ...
    @Test
    /** Prove that Vector2 with fractions serialized then deserialized to a HexCoord is the same as direct conversion (truncating vs 0) */
    fun testFractionalVector2DeserializedAsHexCoord() {
        val json = json()
        val rnd = Random(42)
        fun rndCoord() = (rnd.nextFloat() - 0.5f) * 42f

        repeat(42) {
            val vector = Vector2(rndCoord(), rndCoord())
            val serialized = json.toJson(vector)
