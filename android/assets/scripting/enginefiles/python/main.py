...
A wrapper object does not store any more values than that. When it is evaluated, it uses a simple IPC protocol to request an up-to-date real value from the game's Kotlin code. The `unciv.api.real()` function can be used to manually get a real Python value from a foreign object wrapper.
...
When a Kotlin/JVM object implements a property for size or keys, the Python wrapper for it can also be iterated like a `tuple` or a `dict`, such as in `for` loops and list comprehensions.

---

A **token** is a string that has been generated by `ObjectTokenizer.kt` to represent a Kotlin object.
