...
- You'll need to create a new file ('Create a new file' to the right of the folder name in the UI), and copy into it the contents of [`template.properties`](https://github.com/yairm210/Unciv/blob/master/android/assets/jsons/translations/template.properties).
    - The file name should be the name of your language in English, first letter capitalized, plus the ".properties" extension. If it needs more than one word (e.g. to differentiate variants), use underscores instead of spaces, and fill out the `languageName` field of the LocaleCode entry below.
- Your new language **must** be represented in the [LocaleCode](https://github.com/yairm210/Unciv/tree/master/core/src/com/unciv/models/metadata/GameSettings.kt#L261) enum.
    - You can add this even if you cannot compile. Copy the English line, rename, and change the part in quotes.
    - The entry name must correspond exactly, including case, to the base filename you chose above, unless it contains special characters. In that case, please remove all those, and add a `, languageName="your file name here"` to link the entry to your actual work. Look at the `BrazilianPortuguese` entry and match.
    - Ask an AI for an IETF BCP 47 language tag for your language, or look up the ISO 639-1 code for the language (lower-case), add a dash, and the ISO 3166 code for the nation this is predominantly spoken in (upper-case). Wikipedia can be helpful with these - look for info boxes on the right side.
    - If you get it wrong, the game won't crash, so don't worry overmuch.
    ...
    - The second function is locale-specific representation of numbers. This will fall back to arabic numerals, so if your language uses a different representation, look for a similar language, or ask.
