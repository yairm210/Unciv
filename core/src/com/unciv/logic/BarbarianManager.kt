...
class BarbarianManager {
    ...
    fun placeBarbarianEncampment() {
        ...
        val viableTiles = fogTiles.filter {
            !it.isImpassible()
                    && it.resource == null
                    && it.terrainFeatures.none { feature -> gameInfo.ruleSet.terrains[feature]!!.hasUnique(UniqueType.RestrictedBuildableImprovements)}
    ...
    private fun notifyCivsOfBarbarianEncampment(tile: TileInfo) {
        gameInfo.civilizations.filter {
            it.hasUnique(UniqueType.NotifyBarbCamps)
