...
class UncivFiles(
     ...
     */
    fun saveGamePreview(game: GameInfoPreview, gameName: String, saveCompletionCallback: (Exception?) -> Unit = { if (it != null) throw it }): FileHandle {
        val file = getMultiplayerSave(gameName)
        saveGamePreview(game, file, saveCompletionCallback)
        return file
    }
     ...
     * Only use this with a [FileHandle] obtained by one of the methods of this class!
     */
    fun saveGamePreview(game: GameInfoPreview, file: FileHandle, saveCompletionCallback: (Exception?) -> Unit = { if (it != null) throw it }) {
        try {
