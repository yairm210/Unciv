...
class TileInfoImprovementFunctions(val tileInfo: TileInfo) {
    ...
    fun getImprovementStats(
        ...
        if (tileInfo.hasViewableResource(observingCiv) && tileInfo.tileResource.isImprovedBy(improvement.name)
                && tileInfo.tileResource.improvementStats != null
        )
            stats.add(tileInfo.tileResource.improvementStats!!.clone()) // resource-specific improvement

        val conditionalState = StateForConditionals(civInfo = observingCiv, cityInfo = city, tile = tileInfo)
        ...
        for (unique in improvement.getMatchingUniques(UniqueType.ImprovementStatsForAdjacencies, conditionalState)) {
            ...
            val numberOfBonuses = tileInfo.neighbors.count {
        ...
        fun statsFromTiles(){
            ...
            for (unique in tileUniques + improvementUniques) {
                if (improvement.matchesFilter(unique.params[1])
                        || unique.params[1] == Constants.freshWater && tileInfo.isAdjacentTo(Constants.freshWater)
                        || unique.params[1] == "non-fresh water" && !tileInfo.isAdjacentTo(Constants.freshWater)
                )
                    stats.add(unique.stats)
            }
        }
