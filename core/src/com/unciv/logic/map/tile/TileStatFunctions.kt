...
class TileStatFunctions(val tile: Tile) {
    ...
    fun getTileStats(observingCiv: Civilization?): Stats =
            getTileStats(tile.getCity(), observingCiv)
    ...
    fun getTileStats(
        ...
        val stateForConditionals =
                StateForConditionals(civInfo = observingCiv, city = city, tile = tile)

        if (city != null) {
            var tileUniques = city.getMatchingUniques(
                UniqueType.StatsFromTiles,
                StateForConditionals.IgnoreConditionals
            )
                .filter { city.matchesFilter(it.params[2]) }
            tileUniques += city.getMatchingUniques(
        ...
        if (city != null) {
            var tileUniques = city.getMatchingUniques(
                UniqueType.StatsFromObjectPerCity,
                StateForConditionals.IgnoreConditionals
            )
                .filter { city.matchesFilter(it.params[2]) }
            for (unique in localUniqueCache.get("StatsFromObjectsPerCity", tileUniques)) {
                if (!unique.conditionalsApply(stateForConditionals)) continue
                val tileType = unique.params[1]
                if (!tile.matchesTerrainFilter(tileType, observingCiv)) continue
                stats.add(unique.stats)
            ...
            for (unique in localUniqueCache.get(
                "StatsFromTilesWithout",
                city.getMatchingUniques(
    ...
    fun getTilePercentageStats(
        ...
        if (city != null) {
            ...
            val cachedStatPercentFromObjectUniques = uniqueCache.get(
                UniqueType.StatPercentFromObject.name,
                city.getMatchingUniques(
            ...
            val cachedAllStatPercentFromObjectUniques = uniqueCache.get(
                UniqueType.AllStatsPercentFromObject.name,
                city.getMatchingUniques(
            ...
            val cachedStatPercentFromObjectPerCityUniques = uniqueCache.get(
                UniqueType.SpecificStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(
                    UniqueType.SpecificStatsPercentFromObjectPerCity,
                    StateForConditionals.IgnoreConditionals
                )
            )
                .filter { city.matchesFilter(it.params[3]) }

            for (unique in cachedStatPercentFromObjectPerCityUniques) {
                if (!unique.conditionalsApply(stateForConditionals)) continue
                val tileFilter = unique.params[2]
                if (tile.matchesTerrainFilter(tileFilter, observingCiv))
                    stats[Stat.valueOf(unique.params[1])] += unique.params[0].toFloat()
            ...
            val cachedAllStatPercentFromObjectPerCityUniques = uniqueCache.get(
                UniqueType.AllStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(
                    UniqueType.AllStatsPercentFromObjectPerCity,
                    StateForConditionals.IgnoreConditionals
                )
            )
                .filter { city.matchesFilter(it.params[3]) }
            for (unique in cachedAllStatPercentFromObjectPerCityUniques) {
                if (!unique.conditionalsApply(stateForConditionals)) continue
                val tileFilter = unique.params[1]
                if (!tile.matchesTerrainFilter(tileFilter, observingCiv)) continue
                val statPercentage = unique.params[0].toFloat()
    ...
    fun getImprovementStats(
        ...
        if (city != null) stats.add(
            getImprovementStatsForCity(
                improvement,
                city,
                conditionalState,
                cityUniqueCache
        ...
        for ((stat, value) in getImprovementPercentageStats(
            improvement,
            observingCiv,
            city,
            cityUniqueCache
    ...
    private fun getImprovementStatsForCity(
        ...
        fun statsFromTiles() {
            ...
            val improvementUniques =
                    improvement.getMatchingUniques(
        ...
        fun statsFromObject() {
            ...
            val uniques = cityUniqueCache.get(
                UniqueType.StatsFromObject.name,
                city.getMatchingUniques(
                    UniqueType.StatsFromObject,
                    StateForConditionals.IgnoreConditionals
                )
        ...
        fun statsFromObjectPerCity() {
            // Same as above - cache holds unfiltered uniques for the city, while we use only the filtered ones
            val uniques = cityUniqueCache.get(
                UniqueType.StatsFromObjectPerCity.name,
                city.getMatchingUniques(
                    UniqueType.StatsFromObjectPerCity,
                    StateForConditionals.IgnoreConditionals
                )
    ...
    fun getImprovementPercentageStats(
        ...
        if (city != null) {
            ...
            val allStatPercentUniques = cityUniqueCache.get(
                UniqueType.AllStatsPercentFromObject.name,
                city.getMatchingUniques(
                    UniqueType.AllStatsPercentFromObject,
                    StateForConditionals.IgnoreConditionals
            ...
            val allStatPercentPerCityUniques = cityUniqueCache.get(
                UniqueType.AllStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(
                    UniqueType.AllStatsPercentFromObjectPerCity,
                    StateForConditionals.IgnoreConditionals
            ...
            val statPercentUniques = cityUniqueCache.get(
                UniqueType.StatPercentFromObject.name,
                city.getMatchingUniques(
                    UniqueType.StatPercentFromObject,
                    StateForConditionals.IgnoreConditionals
            ...
            val specificStatsPercentPerCityUniques = cityUniqueCache.get(
                UniqueType.SpecificStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(
                    UniqueType.SpecificStatsPercentFromObjectPerCity,
                    StateForConditionals.IgnoreConditionals
            ...
            for (unique in specificStatsPercentPerCityUniques) {
                if (!improvement.matchesFilter(unique.params[2])) continue
                val stat = Stat.valueOf(unique.params[1])
                stats[stat] += unique.params[0].toFloat()
            }
