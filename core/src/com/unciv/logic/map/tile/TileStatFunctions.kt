...
class TileStatFunctions(val tile: Tile) {
    ...
    fun getTileStats(city: City?, observingCiv: Civilization?,
        ...
        if (city != null) {
            ...
            tileUniques = city.getMatchingUniques(UniqueType.StatsFromObjectPerCity, StateForConditionals.IgnoreConditionals)
                .filter { city.matchesFilter(it.params[2]) }
    ...
    fun getTilePercentageStats(observingCiv: Civilization?, city: City?, uniqueCache: LocalUniqueCache): Stats {
        ...
        if (city != null) {
            ...
            val cachedStatPercentFromObjectPerCityUniques = uniqueCache.get(UniqueType.SpecificStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(UniqueType.SpecificStatsPercentFromObjectPerCity, StateForConditionals.IgnoreConditionals))
            ...
            val cachedAllStatPercentFromObjectPerCityUniques = uniqueCache.get(UniqueType.AllStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(UniqueType.AllStatsPercentFromObjectPerCity, StateForConditionals.IgnoreConditionals))
                .filter { city.matchesFilter(it.params[3]) }
    ...
    private fun getImprovementStatsForCity(
        ...
        fun statsFromObjectPerCity() {
            // Same as above - cache holds unfiltered uniques for the city, while we use only the filtered ones
            val uniques = cityUniqueCache.get(UniqueType.StatsFromObjectPerCity.name,
                city.getMatchingUniques(UniqueType.StatsFromObjectPerCity, StateForConditionals.IgnoreConditionals))
                .filter { city.matchesFilter(it.params[2]) }
                .filter { it.conditionalsApply(conditionalState) }
            for (unique in uniques) {
                if (improvement.matchesFilter(unique.params[1])) {
    ...
    fun getImprovementPercentageStats(
        ...
        if (city != null) {
            ...
            val allStatPercentPerCityUniques = cityUniqueCache.get(UniqueType.AllStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(UniqueType.AllStatsPercentFromObjectPerCity, StateForConditionals.IgnoreConditionals))
                .filter { it.conditionalsApply(conditionalState) }
                .filter { city.matchesFilter(it.params[2]) }
            for (unique in allStatPercentPerCityUniques) {
                if (!improvement.matchesFilter(unique.params[1])) continue
                for (stat in Stat.values()) {
                    stats[stat] += unique.params[0].toFloat()
                }
            ...
            val specificStatsPercentPerCityUniques = cityUniqueCache.get(UniqueType.SpecificStatsPercentFromObjectPerCity.name,
                city.getMatchingUniques(UniqueType.SpecificStatsPercentFromObjectPerCity, StateForConditionals.IgnoreConditionals))
                .filter { it.conditionalsApply(conditionalState) }
                .filter { city.matchesFilter(it.params[3]) }
