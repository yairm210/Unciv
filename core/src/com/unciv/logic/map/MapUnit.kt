...
class MapUnit {
        ...
        when {
            tile.improvementInProgress!!.startsWith("Remove") -> {
                ...
                if (tile.improvementInProgress == "Remove Road" || tile.improvementInProgress == "Remove Railroad")
                    ...
                    // This is not properly working when a tile is containing more than one terrainFeature which can be
                    // removed to get a one-time production bonus since only the first one gets always removed
                    val removedTerrainFeature = tile.terrainFeatures.first { tile.tileMap.gameInfo.ruleSet.terrains[it]!!.uniques
                            .contains("Provides a one-time Production bonus to the closest city when cut down") }
                    tryProvideProductionToClosestCity(removedTerrainFeature)
                    tile.terrainFeatures.remove(removedTerrainFeature)
                }
            }
    ...
    private fun tryProvideProductionToClosestCity(reason: String) {
        val tile = getTile()
        ...
        if (productionPointsToAdd > 0) {
            ...
            civInfo.addNotification("Clearing a [$reason] has created [$productionPointsToAdd] Production for [${closestCity.name}]", closestCity.location, Color.BROWN)
        }
