...
class MapUnitCombatant(val unit: MapUnit) : ICombatant {
    ...
    override fun matchesFilter(filter: String, multiFilter: Boolean) = unit.matchesFilter(filter, multiFilter=multiFilter) // Second parameter is the gameContext, so we need to use multiFilter=multiFilter.
