...
object Battle {
    ...
    private fun postBattleNotifications(
        ...
        if (attacker.getCivInfo() != defender.getCivInfo()) {
            ...
            val attackerDealtString = if (damageDealt != null) " ([-${damageDealt.attackerDealt}] HP)" else ""
            val defenderDealtString = if (damageDealt != null) " ([-${damageDealt.defenderDealt}] HP)" else ""
            val notificationString = attackerString + defenderDealtString + whatHappenedString + defenderString + attackerDealtString
        ...
        for (interceptor in interceptingCiv.getCivUnits()
            ...
            if (interceptor.getTile() in attacker.getCivInfo().viewableTiles) {
                ...
                if (attacker.isDefeated()) {
                    ...
                    val attackerText = "Our [$attackerName] ([-$damage] HP) was attacked by an unknown interceptor"
                    attacker.getCivInfo().addNotification(
                        attackerText, attackedTile.position,
                        attackerName, NotificationIcon.War, interceptorName
                    )
