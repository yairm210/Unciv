...
open class ConsoleBackend(val consoleScope:ConsoleScope) {
    ...
    /* val consoleScope: ConsoleScope
            get() = consoleState.consoleScope */
...
class HardcodedConsoleBackend(consoleScope:ConsoleScope): ConsoleBackend(consoleScope) {
    ...
    override fun exec(command: String): String {
        ...
        when (args[0]) {
                    ...
                    buildingcities = consoleScope.civInfo.cities
                        .filter {
                            ...
                            it.cityConstructions.builtBuildings.any({ building ->
                                consoleScope.gameInfo.ruleSet.buildings[building]!!.requiresResource(searchfor)
                            })
                        }
                        .map { it.name }
                }
            ...
            "godmode" -> {
                if (cheats) {
                    var godmode = if (args.size > 1) args[1].toBoolean() else !(consoleScope.gameInfo.gameParameters.godMode)
                    consoleScope.gameInfo.gameParameters.godMode = godmode
