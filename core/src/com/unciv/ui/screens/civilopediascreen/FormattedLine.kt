...
/* Ideas:
 *    - Fixed link / object columns in [renderFixedIconColumns] so civilopedia text aligns
...
class FormattedLine (
    ...
    val header: Int = 0,
    /** Indentation: 0 = standard; with icons text follows the fixed icon grid.
     *  Without icons, indent 1 aligns to that same text column; each step above adds 30f. */
    val indent: Int = 0,
    ...
    companion object {
        ...
        /** Text offset for icon-less sub-lines: match the fixed icon grid text column, then +indentPad per extra indent. */
        fun sublineTextOffset(indent: Int, iconColumnWidth: Float): Float =
            textColumnOffset(iconColumnWidth) + (indent - 1).coerceAtLeast(0) * indentPad
    ...
    private fun renderSublineText(labelWidth: Float, fontSize: Int, labelColor: Color): Actor {
        ...
        table.defaults().align(Align.left)
        val iconSize = max(minIconSize, fontSize * 1.5f)
        val leading = sublineTextOffset(indent, iconColumnWidth(iconSize))
        if (leading > 0f)
    ...
    private fun renderIcon(table: Table, iconToDisplay: String, iconSize: Float, crossed: Boolean = iconCrossed): Int {
        ...
        // Same footprint for normal and crossed icons (terrain hexes vary intrinsically).
        val actor: Actor = if (crossed) {
