...
class CityButton(val cityView: ForeignCityView, private val tileGroup: TileGroup) : Table(BaseScreen.skin) {
    ...
    private fun setButtonActions() {
        ...
        fun enterCityOrInfoPopup() {
            ...
            if (canEnterCity)
                GUI.pushScreen(CityScreen(CityView(cityView.getCity(), GUI.getWorldScreen().selectedCiv)))
        ...
        if (unitTable.selectedCity != cityView.getCity() && unitTable.selectedUnit?.currentTile != cityView.getCenterTile() && unitTable.selectedSpy == null)
            moveButtonUp()
    }
    ...
    private fun foreignCityInfoPopup() {
        ...
        val popup = Popup(GUI.getWorldScreen()).apply {
            ...
            if (espionageVisible) addButton("View") { GUI.pushScreen(CityScreen(CityView(cityView.getCity(), GUI.getWorldScreen().selectedCiv))) }
            add().expandX()
            addCloseButton {
