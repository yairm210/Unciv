...
internal class AdvancedTab(
    ...
    private fun CoroutineScope.generateScreenshots(settings: GameSettings, configs: ArrayList<ScreenshotConfig>) {
        ...
        launchOnGLThread {
            ...
            newScreen.mapHolder.onTileClicked(newScreen.gameView.getTile(newScreen.mapHolder.tileMap[-2, 3])) // Then click on Keshik
            if (currentConfig.attackCity)
                newScreen.mapHolder.onTileClicked(newScreen.gameView.getTile(newScreen.mapHolder.tileMap[-2, 2])) // Then click city again for attack table
