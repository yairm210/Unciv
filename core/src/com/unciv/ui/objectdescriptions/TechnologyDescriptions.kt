...
object TechnologyDescriptions {
    ...
    fun getDescription(technology: Technology, viewingCiv: Civilization): String = technology.run {
        ...
        if (enabledUnits.any()) {
            ...
            for (unit in enabledUnits)
                lineList += " • ${unit.name.tr()} (${unit.getShortDescription(exclude=technology::uniqueIsRequirementForThisTech)})\n"
        }
        ...
        if (regularBuildings.isNotEmpty()) {
            ...
            for (building in regularBuildings)
                lineList += " • ${building.name.tr()} (${building.getShortDescription(filterUniques=technology::uniqueIsNotRequirementForThisTech)})\n"
        }

        if (wonders.isNotEmpty()) {
            lineList += "{Wonders enabled}: "
            for (wonder in wonders)
                lineList += " • ${wonder.name.tr()} (${wonder.getShortDescription(filterUniques=technology::uniqueIsNotRequirementForThisTech)})\n"
        }
    ...
    fun getCivilopediaTextLines(technology: Technology, ruleset: Ruleset): List<FormattedLine> = technology.run {
        ...
        if (enabledUnits.any()) {
            ...
            for (unit in enabledUnits)
                lineList += FormattedLine(unit.name.tr(true) + " (" + unit.getShortDescription(exclude=technology::uniqueIsRequirementForThisTech) + ")", link = unit.makeLink())
        }
        ...
        if (wonders.isNotEmpty()) {
            ...
            for (wonder in wonders)
                lineList += FormattedLine(wonder.name.tr(true) + " (" + wonder.getShortDescription(filterUniques=technology::uniqueIsNotRequirementForThisTech) + ")", link = wonder.makeLink())
        }
        ...
        if (regularBuildings.isNotEmpty()) {
            ...
            for (building in regularBuildings)
                lineList += FormattedLine(building.name.tr(true) + " (" + building.getShortDescription(filterUniques=technology::uniqueIsNotRequirementForThisTech) + ")", link = building.makeLink())
        }
