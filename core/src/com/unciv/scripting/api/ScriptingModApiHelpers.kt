...
object ScriptingModApiHelpers {
    ...
    fun lambdifyReadPathcode(instance: Any?, pathcode: String): () -> Any? {
        ...
        // Lambda generated by using lambdifyReadPathCode (or something in Mappers) from inside a lambdifyReadPathCode() during a JVM-space callback could have incorrect names, but oh well?
    }
    ...
    // Due to potential and purpose of being used in GUI callbacks, returns from all the lambdafication functions suppress exceptions and show an error dialog instead.
    // This function calls one of those lambdas without the universal exception catching, in case a script wants to use a JVM-space lambda itself and be able to catch JVM-space propagated exceptions for whatever reason.
    fun callLambdaAllowException(func: () -> Unit?) = func.unwrapped()()
