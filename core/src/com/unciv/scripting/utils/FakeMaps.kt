...
interface StatelessMap<K, V>: Map<K, V> { // TODO: Move this.
    companion object {
        fun noStateError(): Nothing = throw(UnsupportedOperationException("Cannot access backing state of ${this::class.simpleName} by default."))
    }
...
class LazyMap<K, V>(val func: (K) -> V, val exposeState: Boolean = false): StatelessMap<K, V>, InvokableMap<K, V> {
    ...
    override val entries get() = if (exposeState) backingMap.entries else StatelessMap.noStateError()
    override val keys get() = if (exposeState) backingMap.keys else StatelessMap.noStateError()
    override val values get() = if (exposeState) backingMap.values else StatelessMap.noStateError()
    override val size get() = if (exposeState) backingMap.size else StatelessMap.noStateError()
    override fun containsKey(key: K) = if (exposeState) backingMap.containsKey(key) else StatelessMap.noStateError()
    override fun containsValue(value: V) = if (exposeState) backingMap.containsValue(value) else StatelessMap.noStateError()
    override fun isEmpty() = if (exposeState) backingMap.isEmpty() else StatelessMap.noStateError()
}
