...
class Building : RulesetStatsObject(), INonPerpetualConstruction {
    ...
    fun getStats(city: City,
        ...
        for ((stat, value) in getYieldPercentageStats(city, localUniqueCache))
        {
    ...
    fun getYieldPercentageStats(city: City?, localUniqueCache: LocalUniqueCache = LocalUniqueCache(false)): Stats {
        val stats = Stats()
        if (city == null)
            return stats

        for (unique in localUniqueCache.get("SpecificStatsPercentFromObjectPerCity", city.getMatchingUniques(UniqueType.SpecificStatsPercentFromObjectPerCity))) {
